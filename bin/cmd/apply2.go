...
func init() {
	apply2Cmd.Flags().StringP("configuration", "f", "", "Release configuration yaml file")
	RootCmd.AddCommand(apply2Cmd)
}

var apply2Cmd = &cobra.Command{
	Use:   "apply2",
...
// apply entry point
//
func apply2Handler(cmd *cobra.Command, args []string) error {
	// fetch configuration
	configurationYaml, err := cmd.Flags().GetString("configuration")
	if err != nil {
		return err
	}

	resourceList, err := kubectl.ParseLocalFile(configurationYaml)
	if err != nil {
		return err
	}
